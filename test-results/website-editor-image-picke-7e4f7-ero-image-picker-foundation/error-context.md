...
- Name: website-editor-image-picker.spec.js >> tenant website editor exposes hero image picker foundation
- Location: tests/browser-audits/website-editor-image-picker.spec.js:13:1
...
```ts
  ...
  11 | }
  12 | 
  13 | test('tenant website editor exposes hero image picker foundation', async ({ page }) => {
  14 |   await login(page);
...
> 18 |   await page.getByRole('button', { name: /^Add Hero Banner$/ }).first().click();
     |                                                                         ^ Error: locator.click: Test timeout of 60000ms exceeded.
  19 |   await expect(page.locator('body')).toContainText(/Hero image|Desktop hero image|Upload image from device|Choose an image file from your computer/);
  20 |   await expect(page.locator('[data-editor-image-upload-input]').first()).toBeAttached();
  21 | });
  22 | 
```