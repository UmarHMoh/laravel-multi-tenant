...
test('tenant website builder can add edit and save nested blocks', async ({ page }) => {
  ...
  await expect(page.locator('body')).toContainText('Blocks inside section');

  await page.getByRole('button', { name: /Add Rich Text/i }).click();
  await page.getByRole('button', { name: /Add Feature Card/i }).click();

  const blockHeading = page.locator('[data-block-setting-id="heading"]').first();
  await expect(blockHeading).toBeVisible();
  await blockHeading.fill('S40 Browser Feature Card');

  const blockText = page.locator('[data-block-setting-id="text"]').first();
  await expect(blockText).toBeVisible();
  await blockText.fill('Nested block content saved from browser test.');
  ...
  await expect(page.locator('body')).toContainText('Heading: S40 Browser Feature Card');

  await page.reload();
  await expectHealthy(page);
  await expect(page.locator('body')).toContainText('S40 Browser Feature Card');
});
