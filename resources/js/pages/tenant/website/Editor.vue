...
const filteredSectionSchemas = computed(() => {
  ...
  if (!query || query.length < 3) {
    return props.sectionSchemas
  }

  const matches = props.sectionSchemas.filter((schema) => sectionMatchesSearch(schema, query))

  return matches.length ? matches : props.sectionSchemas
})
const sectionCategoryOptions = computed(() => {
  const categories = props.sectionSchemas.map((schema) => schema?.category || 'General').filter(Boolean)
...
const favoriteSectionSchemas = computed(() => props.sectionSchemas.filter((schema) => isFavoriteSectionType(schema?.type)))
...
function schemaForPreset(preset) { return props.sectionSchemas.find((schema) => schema?.type === preset?.type) }
  ...
  <div class="flex min-h-screen flex-col bg-slate-100 text-gray-900">
    <header class="sticky top-0 z-40 border-b bg-white shadow-sm">
      <div class="flex min-h-16 flex-col gap-3 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        Homepage draft saved or published.
      </div>
    </header>
        ...
        <div class="h-full max-h-[calc(100vh-4rem)] overflow-y-auto p-4">
          ...
          <section class="mt-6 space-y-3">
            <div v-for="(section, index) in form.sections" :key="section.id" class="rounded-xl border p-3" :class="selectedSectionId === section.id && !selectedBlockId ? 'ring-2 ring-gray-900' : ''">
              ...
              <div class="mt-3 grid grid-cols-2 gap-2">
                ...
                <button type="button" class="min-h-9 rounded-lg border px-2 text-xs font-bold" @click="toggleSection(index)">{{ section.hidden ? 'Show section' : 'Hide section' }}</button>
                <button type="button" class="col-span-2 min-h-9 rounded-lg border border-red-200 px-2 text-xs font-bold text-red-700" @click="removeSection(index)">Remove</button>
              </div>
            </div>
          ...
          <section class="mt-6 rounded-2xl border bg-white p-4">
            ...
            <div class="mt-4 rounded-xl border bg-white p-3" data-section-favorites data-section-favorites-panel-real>
              ...
              <div class="mt-3 grid gap-2">
                <button
                  v-for="schema in props.sectionSchemas"
                  :key="`pin-${schema.type}`"
                  type="button"
            ...
            <div class="mt-4 rounded-xl border bg-white p-3" data-section-presets>
              ...
              <div class="mt-3 grid gap-2">
                <button
                  ...
                  :disabled="!schemaForPreset(preset)"
                  @click="addSectionPreset(preset)"
                >
            ...
            <div class="mt-4 space-y-3">
              <div v-for="section in visibleSectionSchemas" :key="section.type" class="rounded-xl border p-3">
                ...
                <button type="button" class="mt-3 min-h-10 w-full rounded-lg bg-gray-900 px-3 text-sm font-bold text-white" @click="addSection(section)">Add {{ section.name }}</button>
              </div>
            </div>
          </section>
        </div>
                ...
                <section v-for="section in visiblePreviewSections" :key="section.id" class="cursor-pointer p-8" :class="selectedSectionId === section.id && !selectedBlockId ? 'ring-2 ring-gray-900 ring-inset' : ''" @click.stop="selectSection(section)">
                  <template v-if="section.type === 'hero'">
                    <div class="relative overflow-hidden rounded-3xl bg-gray-950 p-10 text-white">
                      <img v-if="section.settings?.image_url" :src="section.settings.image_url" alt="" class="absolute inset-0 h-full w-full object-cover opacity-70" />
                      <div class="relative">
      ...
      <aside v-if="rightSidebarOpen" class="border-l bg-white">
        <div class="h-full max-h-[calc(100vh-4rem)] overflow-y-auto p-4" data-editor-right-sidebar-restored>
          ...
          <div v-else-if="selectedSection" class="mt-5 space-y-5">
            ...
            <label v-for="setting in selectedSchema?.settings || []" :key="setting.id" class="block" :data-setting-id="setting.id">
              <span class="mb-1 block text-xs font-bold uppercase tracking-wide text-gray-500">{{ setting.label }}</span>

              <textarea v-if="setting.type === 'textarea'" v-model="selectedSection.settings[setting.id]" class="min-h-24 w-full rounded-lg border px-3 py-2 text-sm"></textarea>

              <input v-else-if="setting.type === 'number'" v-model="selectedSection.settings[setting.id]" type="number" class="min-h-10 w-full rounded-lg border px-3 py-2 text-sm" />

              <select v-else-if="setting.type === 'select'" v-model="selectedSection.settings[setting.id]" class="min-h-10 w-full rounded-lg border px-3 py-2 text-sm">
                <option v-for="option in setting.options || []" :key="option.value" :value="option.value">{{ option.label }}</option>
              </select>

              <select v-else-if="setting.type === 'product'" v-model="selectedSection.settings[setting.id]" class="min-h-10 w-full rounded-lg border px-3 py-2 text-sm">
              ...
              <div v-else-if="setting.type === 'checkbox'" class="flex items-center gap-2">
                <input v-model="selectedSection.settings[setting.id]" type="checkbox" class="h-4 w-4 rounded border-gray-300" />
                <span class="text-sm text-gray-600">Enabled</span>
              </div>

              <div v-else-if="setting.type === 'image'" class="space-y-2">
                <input v-model="selectedSection.settings[setting.id]" type="text" class="min-h-10 w-full rounded-lg border px-3 py-2 text-sm" placeholder="Paste image URL" />
                <input type="file" accept="image/png,image/jpeg,image/webp,image/gif" class="block w-full text-xs" data-editor-image-upload-input @change="uploadEditorImage($event, (url) => selectedSection.settings[setting.id] = url)" />
              </div>

              <select v-else-if="setting.type === 'url'" v-model="selectedSection.settings[setting.id]" class="min-h-10 w-full rounded-lg border px-3 py-2 text-sm">
                <option v-for="option in normalisedLinkOptions" :key="option.value" :value="option.value">{{ option.label }}</option>
                <option value="#">Custom URL...</option>
              </select>

              <input v-else v-model="selectedSection.settings[setting.id]" type="text" class="min-h-10 w-full rounded-lg border px-3 py-2 text-sm" />
            </label>
              ...
              <div class="mt-3 space-y-3">
                <div v-for="(block, index) in selectedSection.blocks || []" :key="block.id" class="rounded-xl border bg-white p-3">
                  ...
                  <div v-if="selectedBlockId === block.id" class="mt-3 space-y-3">
                    <label v-for="setting in selectedBlockSchema?.settings || []" :key="setting.id" class="block" :data-block-setting-id="setting.id">
                      <span class="mb-1 block text-xs font-bold uppercase tracking-wide text-gray-500">{{ setting.label }}</span>
